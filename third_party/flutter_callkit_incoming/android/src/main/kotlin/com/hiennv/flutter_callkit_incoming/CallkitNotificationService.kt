...
class CallkitNotificationService : Service() {
    ...
    private fun startForeground(notificationId: Int, notification: Notification, isVideo: Boolean) {
        // Symon's Kitchin: this is an order alert styled as a call — it never
        // records audio or uses the camera. Use the PHONE_CALL foreground-service
        // type ONLY, so the app doesn't need the MICROPHONE/CAMERA FGS permissions
        // (which Google Play otherwise requires a video justification for).
